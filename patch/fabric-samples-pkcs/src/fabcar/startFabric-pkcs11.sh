...
pushd ../test-network
./network.sh down -pkcs11
./network.sh up createChannel -capkcs11 -s couchdb
./network.sh deployCC_pkcs11  -ccn fabcar -ccv 1 -cci initLedger -ccl ${CC_SRC_LANGUAGE} -ccp ${CC_SRC_PATH}
popd
